...
const uiText = {
  zh: {
    docTitle: "2026 Commencement + Family Trip",
    subtitle:
      "以畢業典禮為核心，整理波士頓、費城、華盛頓特區與紐約的住宿、交通、餐廳、景點、租車、停車、票券與注意事項。",
    defaultNote: "",
    quickNav: "快速導覽",
  ...
  en: {
    docTitle: "2026 Commencement + Family Trip",
    subtitle:
      "A practical family itinerary for Boston, Philadelphia, Washington, DC, and New York, centered on commencement logistics, pacing, transportation, dining, maps, car rental, parking, tickets, and key travel notes.",
    defaultNote: "",
    ...
    sourceDirectory: "Official references and confirmations",
    intensity: "Intensity",
    theme: "Theme",
    avoid: "Do not add",
    mapLinks: "Map links",
    readerMode: "Family trip",
    switchToEnglish: "English",
    switchToChinese: "中文",
...
const routeNodes = [
  { id: "bos", label: { zh: "波士頓", en: "Boston" }, date: "4/26 至 5/2", icon: "graduation", note: { zh: "住宿、畢業典禮、哈佛、自由之路、水岸線", en: "Hotel, ceremonies, Harvard, Freedom Trail, waterfront" } },
  ...
  { id: "phl", label: { zh: "費城", en: "Philadelphia" }, date: "5/2 至 5/5", icon: "landmark", note: { zh: "4211 Suites、UPenn、還車、上 Amtrak", en: "4211 Suites, UPenn, car return and Amtrak" } },
  ...
  { id: "nyc", label: { zh: "紐約", en: "New York" }, date: "5/5 至 5/11", icon: "train", note: { zh: "住宿與航班未定，分區模組待排序", en: "Hotel and flight pending, geographic modules first" } },
];
...
const ceremonies = [
  ...
  },
  {
    ...
    maps: [
      ...
      { label: { zh: "Northeastern 至 Leader Bank Pavilion", en: "Northeastern to Leader Bank Pavilion" }, url: mapDirections(addresses.northeastern, addresses.leader) },
...
const riskGroups = [
  ...
  },
  {
    ...
    items: [
      { zh: "紐約住宿地址待確認，因此目前先採用分區模組安排。", en: "New York lodging is pending, so the plan uses geographic modules for now." },
      { zh: "5/11 機場與航班時間待確認。", en: "May 11 airport and flight time are pending." },
...
function App() {
  ...
  return (
    <main
      className="min-h-screen bg-[#FCFAF2] text-slate-900"
      style={{
        fontFamily:
          lang === "zh"
            ? '"PingFang TC", "Noto Sans CJK TC", "Noto Sans TC", "Microsoft JhengHei", system-ui, sans-serif'
            : 'Inter, "SF Pro Text", "Segoe UI", system-ui, sans-serif',
      }}
    >
        ...
        <div className="mx-auto max-w-7xl px-3 py-7 md:px-6 md:py-10 lg:py-12">
          <div className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr] lg:items-end">
            <div className="min-w-0">
              ...
              {t.defaultNote ? <p className="mt-2 text-sm leading-6 text-slate-600">{t.defaultNote}</p> : null}
            ...
            <Card className="border-[#d9ccb4] bg-white/85 shadow-sm">
              ...
              <CardContent className="space-y-3 text-sm leading-6 text-slate-700">
                <p>{lang === "zh" ? "優先確認兩件事：5/2 早上取車是否可行，以及 4211 Suites 停車安排。這兩項會直接影響 American Dream、費城住宿與 DC 開車日。" : "Confirm two items first: whether an early May 2 car pickup is available and how parking works at 4211 Suites. These affect American Dream, Philadelphia lodging, and the DC driving day."}</p>
                <p>{lang === "zh" ? "紐約段先保留為分區模組。等飯店地址與 5/11 航班資訊確認後，再決定每日順序與機場交通。" : "The New York segment stays modular until the hotel address and May 11 flight details are confirmed. Once available, reorder the daily modules and finalize airport transportation."}</p>
              </CardContent>
            </Card>
          </div>
        </div>
      ...
      <div className="mx-auto max-w-7xl px-3 py-6 md:px-6 md:py-8">
        ...
        {(filter === "all" || filter === "risks") && (
          <section className="mb-9">
            <SectionTitle eyebrow={lang === "zh" ? "注意事項" : "Travel notes"} title={t.riskBoard}>
              {lang === "zh" ? "此區集中整理出發前需要確認的事項，以及容易造成延誤或誤判的細節。" : "This section keeps the details that should be confirmed before departure and the points most likely to cause delays or confusion."}
            </SectionTitle>
            <div className="grid gap-4 md:grid-cols-2">
        ...
        {(filter === "all" || filter === "sources") && (
          <section className="mb-9">
            <SectionTitle eyebrow={lang === "zh" ? "參考資訊" : "References"} title={t.sourceDirectory}>
              {lang === "zh" ? "以下列出主要官方資訊來源。出發前仍需再確認官方頁面。" : "The main official references are listed below. Check official pages again before departure."}
            </SectionTitle>
